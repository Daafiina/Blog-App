          ...
          <div className='md:flex items-center justify-between bg-white py-4 md:px-10 px-7'>
            ...
            <div onClick={() => setOpen(!open)} className='absolute right-8 top-6 cursor-pointer md:hidden w-7 h-7'>
             ...
             {linksItem.map(({path, label}) => (
                <li className="mr-4 md:mr-8 text-2xl" key={path}>
                  <Link className="text-gray-800 hover:text-blue-600 transition-colors duration-300" to={path}>{label}</Link>
                </li>
              ))}
                <button className='btn bg-blue-600 text-2xl text-white md:ml-8 font-semibold px-3 py-1 rounded duration-500 md:static'>Login</button>
            </ul>
          </div>
        </div>
    </nav>
